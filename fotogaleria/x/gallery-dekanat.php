...
$mainFolder    = '../_fotoalbumy/kostoly';		// folder where your albums are located - relative to root
$albumsPerPage = '10';			// number of albums per page
$itemsPerPage  = '15';			// number of images per page    
...
// display list of albums
	...
	foreach($folders as $kostol) {

		if(!in_array($kostol, $ignore)) {

			array_push( $albums, $kostol );

			$caption = substr($kostol,0,44);
			array_push( $captions, $caption );

			$rand_dirs = glob($mainFolder.'/'.$kostol.'/thumbs/*.*', GLOB_NOSORT);
	...
	if( count($albums) == 0 ) {
		...
		echo "\n\t\t\t\t\t\t\t";
		echo '<div class="text-left"><span class="title">Fotogaléria kostolov a kláštorov dekanátu</span></div>';
		echo "\n\t\t\t\t\t\t\t";
		...
		//for( $i=$start; $i>=$ende; $i-- ) {	
			...
			if( isset($albums[$i]) ) {
				echo "\n\n\t\t\t\t\t\t";
				echo '<a href="'. replace_whitespace($_SERVER['PHP_SELF']). '?kostol='. replace_whitespace($albums[$i]) .'">';
				//echo '<a href="kostol='. replace_whitespace($albums[$i]) .'">';
				echo "\n\t\t\t\t\t\t\t";
...
// display photos in album
	$src_folder = $mainFolder.'/'.$_GET['kostol'];
	...
	echo '<div class="titlebar">';
	echo "\n\t\t\t\t\t\t\t\t";
	echo '<div class="text-left"><span class="title">'. $_GET['kostol'] .'</span>';
	echo "\n\t\t\t\t\t\t\t\t\t";
	//echo '<a class="nazad float-right" href="'.$_SERVER['PHP_SELF'].'"><strong>Všetky albumy</strong></a></div>';
	...
	for( $i=$start; $i<$start + $itemsPerPage; $i++ ) {
		if( isset($files[$i]) && is_file( $src_folder .'/'. $files[$i] ) ) { 
			...
			echo '<div class="thumb-wrapper">';
			echo "\n\t\t\t\t\t\t\t\t\t\t";
			echo '<img rel=\'noindex\' src="'. replace_whitespace($src_folder) .'/thumbs/'. replace_whitespace($files[$i]) .'" width="'.$thumb_width.'" alt="'. $_GET['kostol'] .'-'. $i .'" />';
			echo "\n\t\t\t\t\t\t\t\t\t";
			echo '</div>';
			echo "\n\t\t\t\t\t\t\t\t";
			echo '</div>';
	...
	$urlVars = "kostol=".replace_whitespace($_GET['kostol'])."&amp;";
	print_pagination($numPages,$urlVars,$currentPage);
	} // end else
